...
export class TvShowsService {
  ...
  public text : string = "Shared info";
  ...
  public onSubmitEdit(edit: FormGroup): void {
    if (edit.valid) {
      if (this.showToEdit !== null) {
        Object.assign(this.showToEdit, edit.value);
      }
  ...
  public sort_shows(): void {
    if (this.orderBy == "name") {
      this._shows.sort(this.sort_by_name);
    } else  {
      this._shows.sort(this.sort_by_date);
    }
    if (this.invert == -1) {
      this._shows.reverse();
    }
